...
Invoke-Test "conversion installs packaged app instead of scratch registration" {
    ...
    if ($scriptText -notmatch [regex]::Escape('"Cert:" + "\CurrentUser"')) {
    ...
    $scratchPackage = [pscustomobject]@{
        InstallLocation = Join-Path $TempRoot ".scratch\codex-arm64-conversion\converted\Codex"
    }
    ...
    $installedPackage = [pscustomobject]@{
        InstallLocation = Join-Path $TempRoot "WindowsApps\OpenAI.Codex.Arm64Dev_26.429.2026.0_arm64__abc"
    }
