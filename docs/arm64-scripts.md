...
- If the Converted App is already installed at the same version and no Store update is available, skip snapshot/conversion/registration and only remove the Store Package so duplicate Codex apps are not left installed.
...
- After the Store package is installed and copied to the snapshot directory, remove the current-user Store package before registering the Converted App.
...
- Strip Store signature/origin artifacts from the snapshot before developer-mode registration of the unpacked Converted App.
...
- Produce an unpacked registered app first; signed MSIX packaging can come after the unpacked app proves the ARM64 conversion.
- Register the unpacked Converted App under a distinct dev identity after the installed Store Package has been removed.
- Register the unpacked Converted App for the current user only.
...
- Report success after current-user unpacked app registration succeeds.
