...
This repository targets the Microsoft Store distributed Codex desktop app, not the Codex CLI. The conversion script snapshots the current-user Store install, replaces the Electron runtime and native modules with ARM64-compatible versions, registers an unpacked current-user app, and removes the Store package so only one Codex app remains installed.
...
- `scripts/convert-codex-store-app-arm64.ps1`
  Converts the Store app snapshot into an unpacked ARM64 Codex desktop app.
...
- Developer Mode or sideloading enabled for unpacked app registration
...
```powershell
pwsh -NoProfile -File .\scripts\convert-codex-store-app-arm64.ps1
```
